        ...
        {showForm && (
          <form onSubmit={handleSubmit} style={{background: "white", color: "#333", padding: "20px", borderRadius: "12px", marginTop: "20px", marginBottom: "20px"}}>
            ...
            <div style={{marginTop: "10px"}}>
              <button type="submit" style={{padding: "10px 20px", background: "#667eea", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", marginRight: "10px"}}>{editingId ? "Update" : "Create"}</button>
            ...
            <div key={e.id} style={{background: "white", color: "#333", padding: "15px", marginBottom: "10px", borderRadius: "8px", border: `2px solid ${e.status === "approved" ? "#10b981" : e.status === "rejected" ? "#ef4444" : "#fbbf24"}`}}>
              <div style={{display: "flex", justifyContent: "space-between", alignItems: "start"}}>
                <div>
                  ...
                  <span style={{display: "inline-block", padding: "2px 8px", borderRadius: "12px", fontSize: "0.9em", background: e.status === "approved" ? "#d1fae5" : e.status === "rejected" ? "#fee2e2" : "#fef3c7", color: e.status === "approved" ? "#065f46" : e.status === "rejected" ? "#7f1d1d" : "#92400e"}}>{e.status.toUpperCase()}</span>
                </div>
                <div style={{display: "flex", gap: "8px"}}>
                  {canEdit(e) && <button onClick={() => handleEdit(e)} style={{padding: "6px 12px", background: "#3b82f6", color: "white", border: "none", borderRadius: "4px", cursor: "pointer"}}>✏️ Edit</button>}
                  {isAdmin && <button onClick={() => handleDelete(e.id)} style={{padding: "6px 12px", background: "#ef4444", color: "white", border: "none", borderRadius: "4px", cursor: "pointer"}}>🗑️ Delete</button>}
                </div>
              </div>
