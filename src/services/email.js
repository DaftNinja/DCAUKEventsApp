...
<body>
  <div class="wrapper">
    <div class="header">
      <div class="logo">theventguide.com</div>
    </div>
    <div class="body">${content}</div>
    <div class="footer">
      theventguide.com · <a href="${SITE_URL}" style="color:#06b6d4;">theventguide.com</a><br/>
      You're receiving this because you're a member of theventguide.com.
    </div>
  </div>
</body>
</html>`;
...
export async function sendNewEventNotification({ event, recipients }) {
  ...
  const content = `
    ...
    <p>A new event has been added to the theventguide.com calendar:</p>
    <div class="event-card">
      <div class="event-title">${event.title}</div>
      ${event.organiser ? `<div class="event-meta">🏢 ${event.organiser}</div>` : ""}
      <div class="event-meta">📅 ${formatEventDate(event.startDate)}</div>
...
export async function sendEventApproved({ event }) {
  ...
  const content = `
    ...
    <p>Great news — your event submission has been reviewed and <strong>approved</strong> by the theventguide.com team. It's now live and visible to the community.</p>
...
export async function sendEventRejected({ event }) {
  ...
  const content = `
    ...
    <p>Hi ${event.organiser || "there"},</p>
    <p>Thank you for submitting your event to theventguide.com. Unfortunately, after review, we're unable to publish the following event at this time:</p>
    <div class="event-card">
      <div class="event-title">${event.title}</div>
      <div class="event-meta">📅 ${formatEventDate(event.startDate)}</div>
    </div>
